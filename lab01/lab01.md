...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Информацию о браузере, ОС, принимаемых форматах, кодировках и языках
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да
   - Это можно увидеть по размеру содержимого (371 байт) и по его содержанию (10 строк)
     
   ![](../img/1.2.2.png)
   
4. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - За заголовком следует дата и время последней модификации, полученные из первого ответа
     
     ![](../img/1.2.3.png)
     
5. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - 1
   - 1 (из http), 415 (из всех)
  
    ![](../img/1.3.1.png)
   
3. Какой номер пакета в трассировке содержит код состояния и фразу, связанные с ответом
   на HTTP-запрос GET?
   - 2 (из http), 431 (из всех)
     
     ![](../img/1.3.2.png)
     
4. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - 3
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Параллельно, так как ответ на более ранний запрос картинки пришёл позже
