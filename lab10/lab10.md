...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Version, Header Length, Differentiated Services Field, Total Length, Flags, Fragment Offset, Protocol, Header checksum status, Source Address, Destination Address, Stream index
      - Наверное все вышеперечисленные, но из них лично мне очевидно, что: Version, Header Length, Total Length, Protocol, Source Address, Destination Address
      - Identification, Time to Live, Header Checksum (очевидно, что TTL, ну остальные видимо тоже, раз изменяются)
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Увеличиваются на один с каждым запросом
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Time to Live: 64 у всех. Identification: растёт на единицу с каждым пакетом.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - У меня за время время трассировки не поступило ни одного эхо-запроса на мой компьютер (да и там должны быть ответы, но и их не поступило, потому что всё потерялось). В общем будем считать, что имелись в виду запросы с моего компьютера, тем более в ответах должно быть так же. (Да и кроме них у меня ничего нет...)
      - Да, 3 фрагмента создалось
