...
Ниже будет два скрина работы на случайно сгенерированных сетях на 3 и 10 узлов. Для проверки на существующей сети нужно удалить вызов generate_network в main. -1 в метрике указывает на то, что узел не достижим из данного. В случайной сети рёбра ориентированы, однако можно их продублировать в обе стороны, всё будет работать как в неориентированном случае. Вероятность каждого ребра в генерации = 0.5, поэтому в примере на 3 могут быть недостижимые узлы, а в примере на 10 пути довольно короткие.
