...
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   ...
   - Все по 2 байта (написано снизу и отображается справа, на скрине привожу один, но я все 4 проверил)
  
   ![](../img/6.02.png)
     
3. Значение в поле Length (Длина) – это длина чего?
   - Сумма хэдера и данных (на скрине снизу)

   ![](../img/6.03.png)
     
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - Длина занимает 2 байта => $2^16\ -\ 1$ байт - 8 байт на заголовки
5. Чему равно максимально возможное значение номера порта отправителя?
   - По той же логике $2^16\ -\ 1$ = 65535
...
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   ...
   - В запросе: Src Port: 51977, Dst Port: 53
   - В ответе: Src Port: 53, Dst Port: 51977
   - Поменялись местами, что логично
...
Я создал сервер, подключился к нему, создал на нём папку, скачал её, создал в ней локально файл и отправил его на сервер, потом всё поудалял.
