...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Потому что работает на сетевом уровне
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - Checksum, Checksum Status, Identifier (BE), Identifier (LE), Sequence Number (BE), Sequence Number (LE), Data
   - По 2 байта
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Отличается
   - TTL теперь не 128, а увеличивающийся от 1 и дальше
   - Есть поле, указывающее на отсутствие ответа
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Unused: тут просто нули
   - Internet Protocol Version 4, Internet Control Message Protocol: в этих заголовках информация о пакете запроса
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - У них другой тип (0 вместо 11), нет трёх полей из прошлого пункта, есть время ответа
   - Они просто дошли до цели и сказали что всё ок, вместо того чтобы отправить ошибку
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Да, канал 216.239.63.57 — 192.178.80.79
   - Они не проименованы, но можно предположить, что это из Европы в США (оказалось наоборот, из США в Европу, ну у меня вообще маршрут странный там)
