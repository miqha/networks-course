...
1. Первый пункт:
   ...
   1.1. Воспользуемся леммой ферма и найдём все точки с нулевой производной на отрезке $[0, 1]$. В одной из них (либо на концах) будет глобальный максимум на отрезке.
   
   1.2. При $p = 0$ или $p = 1$ функция равна 0.

   1.3. Производная по $p$: $(N \cdot p (1 - p)^{N - 1})' = N\cdot ((1 - p)^{N-1} - (N-1)\cdot p\cdot (1 - p)^{N-2}) = N\cdot (((1 - p) - (N-1)\cdot p)\cdot (1 - p)^{N-2}) = N\cdot ((1 - N\cdot p)\cdot (1 - p)^{N-2}).$

   1.4. Нули у неё в $p = 1$ и в $p = \frac{1}{N}$. Первое мы уже считали выше, а при втором значении функция явно положительная, то есть это именно максимум.

   1.5. Ответ: $p = \frac{1}{N}$.
   
2. Второй пункт:
   
   2.1. Подставляем $p = \frac{1}{N}$ в $N \cdot p (1 - p)^{N - 1}$ и получаем: $N \cdot \frac{1}{N} (1 - \frac{1}{N})^{N - 1} = (1 - \frac{1}{N})^{N - 1}$.

   2.2. Воспользуемся замечательным пределом для числа $e$: $(1 - \frac{1}{N})^{N - 1} = \frac{(1 - \frac{1}{N})^{N}}{1 - \frac{1}{N}} \xrightarrow[]{} (N\rightarrow\infty) \  \frac{e^{-1}}{1}  = \frac{1}{e} $.
...
1. Вероятность события, что А решил передать информацию и ему это удалось = вероятности того, что он решил отправить * вероятность того, что канал был свободен = $p(1 - p)^3$.
2. Тут 3 непересекающихся события аналогичных событию из первого пункта, так что складываем и получаем: $3p(1 - p)^3$.
3. Тут три независимых события подряд: неудача в квант 1, неудача в квант 2, удача в квант 3. Значит надо перемножить вероятности. Вероятность удачи = $4p(1 - p)^3$. Вероятность неудачи = 1 - вероятность удачи = $1 - 4p(1 - p)^3$. Итого: $4p(1 - p)^3 \cdot (1 - 4p(1 - p)^3)^2$.
4. При наличии $N$ активных узлов эффективность дискретного протокола ALOHA равна $N \cdot p (1 - p)^{N - 1}$. При $N = 4$: $4p(1 - p)^3$.
