...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence Number: 0 (relative sequence number), Sequence Number (raw): 3829385653
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Sequence Number: 0 (relative sequence number), Sequence Number (raw): 3224881539
   - Acknowledgment Number: 1 (relative ack number), Acknowledgment number (raw): 3829385654
   - Буквально Seq. Number из пункта выше + 1 (Next rel. Seq. Number 1 кстати в прошлом пакете даже был указан явно)
   - Снова стоят флаги, на этот раз два (syn и ack)
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Sequence Number: 1 (relative sequence number), Sequence Number (raw): 3829385654
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Время отправки и получения пакета определялись вручную, RTT определялось строчкой в информации о пакете (низ скрина), на первый и третий пакеты я ответы не нашёл, потерялись видимо
   

      | Номер | Отправка | Ответ | RTT
      |-|-|-|-
      |1   | 13.915324| -        | -
      |729 | 13.916110| 14.070343| 0.1550
      |2189| 13.916110| -        | -
      |3649| 13.916110| 14.072820| 0.1567
      |5109| 13.916110| 14.072820| 0.1567
      |6569| 13.916110| 14.072820| 0.1567
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Возьмём Windowsize пакетов (65280 байт у всех) и поделим на RTT (почти у всех 0.1567 с)
   - Ответ: 65280 / 0.1567 ≈ 416592 байт/с
